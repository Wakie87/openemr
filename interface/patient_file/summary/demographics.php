...
<body class="body_top" style="margin: 8px;">
	<div class="container-fluid">
			...
			<?php
			 ...
			 if ($thisauth) {
			  ...
			  if (acl_check('admin', 'super')) {
			   echo "<td style='padding-left:1em;'><a class='btn iframe' href='../deleter.php?patient=" . 
				htmlspecialchars($pid,ENT_QUOTES) . "'>" .
				"<span>".htmlspecialchars(xl('Delete'),ENT_NOQUOTES).
				"</span></a></td>";
			  }
			  if($GLOBALS['erx_enable']){
				echo '<td style="padding-left:1em;"><a class="btn" href="../../eRx.php?page=medentry" onclick="top.restoreSession()">';
				echo "<span>".htmlspecialchars(xl('NewCrop MedEntry'),ENT_NOQUOTES)."</span></a></td>";
				echo '<td style="padding-left:1em;"><a class="btn iframe1" href="../../soap_functions/soap_accountStatusDetails.php" onclick="top.restoreSession()">';
			  ...
			  if($GLOBALS['portal_onsite_enable'] && $GLOBALS['portal_onsite_address']){
				...
				if ($portalStatus['allow_patient_portal']=='YES') {
				  ...
				  echo "<td style='padding-left:1em;'><a class='btn iframe small_modal' href='create_portallogin.php?portalsite=on&patient=" . htmlspecialchars($pid,ENT_QUOTES) . "' onclick='top.restoreSession()'>";
			  ...
			  if($GLOBALS['portal_offsite_enable'] && $GLOBALS['portal_offsite_address']){
				...
				if ($portalStatus['allow_patient_portal']=='YES') {
				  ...
				  echo "<td style='padding-left:1em;'><a class='btn iframe small_modal' href='create_portallogin.php?portalsite=off&patient=" . htmlspecialchars($pid,ENT_QUOTES) . "' onclick='top.restoreSession()'>";
				  if (empty($portalLogin)) {
		...
		<div class="row-fluid span12" style="margin-top:10px">
			<div class="btn-group">
					  <a class="btn" href="../history/history.php" onclick="top.restoreSession()" ><?php echo xl('History','e');?></a>
					  <a class="btn" href="../report/patient_report.php" onclick="top.restoreSession()" >Report</a>
					  <a class="btn" href="../../../controller.php?document&list&patient_id=<?php echo $pid;?>" onclick="top.restoreSession()" >Documents</a>
					  <a class="btn iframe large_modal" href="../transaction/transactions.php" onclick="top.restoreSession()" >Transactions</a>
					  <a class="btn" href="stats_full.php?active=all" onclick="top.restoreSession()">Issues</a>
			</div>

		</div>


<div class="row-fluid span12" style='margin-top:10px'> <!-- start main content div -->
	...
	$widgetLabel = xl("billing");
	$widgetButtonLabel = xl("Edit");
	$widgetButtonLink = "return newEvt();";
	$widgetButtonClass = "";
	$linkMethod = "javascript";
	$bodyClass = "accordion-body collapse in";
	$widgetAuth = false;
	$fixedWidth = true;
	...
	<?php
	 ...
	 // Show current balance and billing note, if any.
	  echo "<div class='accordion-inner'><table border='0'><tr><td>" .
	  ...
	  echo "</table></td></tr></td></tr></table></div>";
	 }
	?>		
			</div>
