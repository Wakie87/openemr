 ...
 */
    function privStatement($sql,$params=null)
    {
        if(is_array($params))
        {
            $recordset = $this->getPrivDB()->Execute( $sql, $params );
        }
        else
        {
            $recordset = $this->getPrivDB()->Execute( $sql );
        }
        if ($recordset === false) {
          ...
          // need them if there is a database problem.
            ...
            error_log("Executing as user:" .$this->getPrivDB()->user." Statement failed:".$sql.":". $GLOBALS['last_mysql_error']
