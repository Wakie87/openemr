...
require_once(dirname(__FILE__) . '/formdata.inc.php');
    ...
    function xl($constant,$mode='r',$prepend='',$append='') {
      ...
      if ($lang_id == 1 && !empty($GLOBALS['skip_english_translation'])) {
        ...
        $sqlc = new sql();
        $res = $sqlc->sqlStatementNoLog($sql,array($lang_id,$constant));
        $row = $sqlc->SqlFetchArray($res);
    ...
    function getLanguageDir($lang_id) {
        ...
        // get language code
        $sql = new sql();
        $row = $sql->sqlQuery('SELECT * FROM lang_languages WHERE lang_id = ?', array($lang_id));
